/**
 * MoneyControl News Parser
 * Specialized parser for MoneyControl news website
 */

const logger = require('../../utils/logger');

class MoneyControlParser {
  constructor() {
    this.sourceName = 'moneyControl';
    this.baseUrl = 'https://www.moneycontrol.com';
    this.searchUrl = 'https://www.moneycontrol.com/search';

    // MoneyControl specific selectors
    this.selectors = {
      // Search results page
      searchResults: '.search-result, .news-item, .story-item',
      searchResultTitle: 'h1, h2, h3, .title, .headline',
      searchResultLink: 'a',
      searchResultSummary: '.summary, .description, .excerpt, p',

      // Article page
      articleTitle: 'h1.article_title, .article_title h1, .artTitle',
      articleContent: '.article_content, .article-body, .story-content',
      articleMeta: '.article_meta, .article-meta, .story-meta',
      articleDate: '.article_date, .date, .published-date',
      articleAuthor: '.author, .byline, .writer',
      articleTags: '.tags, .keywords, .categories',

      // Stock specific elements
      stockSymbol: '.stock-symbol, .symbol, .ticker',
      stockPrice: '.stock-price, .price, .current-price',
      stockChange: '.stock-change, .change, .price-change',
    };
  }
  ...
  async parseSearchResults(page) {
    try {
      logger.info('Parsing MoneyControl search results');
      ...
      logger.info('MoneyControl search results parsed', {
        resultCount: results.length,
      });

      return results;
    } catch (error) {
      logger.error('Failed to parse MoneyControl search results', {
        error: error.message,
      });
      return [];
    }
  ...
  async parseArticle(page, url) {
    try {
      logger.info('Parsing MoneyControl article', { url });
      ...
      logger.info('MoneyControl article parsed successfully', {
        url,
      ...
      logger.error('Failed to parse MoneyControl article', {
        url,
        error: error.message,
      });
      return null;
  ...
  normalizeDate(dateString) {
    ...
    try {
      // Handle common MoneyControl date formats
