/**
 * BSE Official News Parser
 * Specialized parser for BSE (Bombay Stock Exchange) official website
 */
...
class BSEParser {
  constructor() {
    ...
    this.searchUrl = 'https://www.bseindia.com/search';

    // BSE specific selectors
    this.selectors = {
      // Search results page
      searchResults:
  ...
  async parseSearchResults(page) {
    try {
      logger.info('Parsing BSE search results');
      ...
      logger.info('BSE search results parsed', {
        resultCount: results.length,
      });

      return results;
    } catch (error) {
      logger.error('Failed to parse BSE search results', {
        error: error.message,
      });
      return [];
    }
  ...
  async parseAnnouncement(page, url) {
    try {
      logger.info('Parsing BSE announcement', { url });
      ...
      logger.info('BSE announcement parsed successfully', {
        url,
      ...
      logger.error('Failed to parse BSE announcement', {
        url,
        error: error.message,
      });
      return null;
  ...
  normalizeDate(dateString) {
    ...
    try {
      // Handle common BSE date formats
      ...
      // Try BSE specific format: "Date: 15/01/2024"
      const bseDateMatch = cleanedDate.match(
        /(?:Date:?\s*)?(\d{1,2}[/-]\d{1,2}[/-]\d{4})/
      );
      if (bseDateMatch) {
        const dateParts = bseDateMatch[1].split(/[/-]/);
  ...
  extractStockSymbols(content) {
    ...
    // BSE stock patterns (typically 3-10 characters, all caps)
